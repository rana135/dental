import React from 'react';
import dentist1 from '../../assets/images/dentist1.jpg'

const Banner = () => {
    return (
        <div>
            <div className="hero min-h-screen">
                <div className="hero-content flex-col lg:flex-row-reverse mx-5 ">
                    <img src={dentist1}  alt='' className="max-w-sm rounded-lg shadow-2xl"/>
                    <div>
                        <h1 className="text-5xl font-bold">Friendly Gentle Dentist</h1>
                        <p className="py-6">Our number one goal is to make sure every one of our patients leaves 100% happy and with a bright, white smile on their face!</p>
                        <button className="btn bg-gradient-to-r from-secondary to-primary text-white text-bold">Get Started</button>
                    </div>
                </div>
            </div>
        </div>
    );
};

export default Banner;